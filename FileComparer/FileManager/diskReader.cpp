...
DiskReader::DiskReader(std::string path):
m_diskName(path)
{
}
...
std::string DiskReader::loadAllVolume()
{
	std::string ret;
	...
	{	   
		...
		{    
			...
			if(fileSysBuf[0] == L'N' && fileSysBuf[1] == L'T' && fileSysBuf[2] == L'F' && fileSysBuf[3] == L'S')
			{
				...
				ret.append(WChar2Ansi(szRootName));
...
bool DiskReader::GetVolumeDetail( HANDLE hVol)
{
    bool ret = false;
    DWORD BytesReturned;
