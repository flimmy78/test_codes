/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3002_TIP/TA_BASE/transactive/bus/scada/datapoint_library/src/NumberConversionUtil.cpp $
  * @author:	HoaVu
  * @version:	$Revision: #1 $
  *
...
namespace TA_Base_Bus
{
	...
	int NumberConversionUtil::convertFromINT15_Value2( WORD originalNumber, long bitWidth)
	{
		unsigned int mostSignificantBit = 0;
		unsigned long thirty2BitNumber = 0;
		int converted = 0;
		...
		// get the sign bit of the original number
		originalNumber = originalNumber << (16 - bitWidth);
		mostSignificantBit = originalNumber >> 15;
		originalNumber = originalNumber >> (16 - bitWidth);

		if ( 1 == mostSignificantBit )
		{
			thirty2BitNumber = originalNumber << (32 - bitWidth);
			memcpy(&converted, &thirty2BitNumber, 4);
			converted = converted >> (32 - bitWidth);
			return converted;
		}
		else
		{
	...
	int NumberConversionUtil::convertFromINT15_Value( WORD originalNumber, long bitWidth)
	{
		unsigned int mostSignificantBit = 0;
		unsigned long thirty2BitNumber = 0;
		int converted = 0;
	...
	unsigned int NumberConversionUtil::convertFromUINT15_Value( WORD originalNumber, long bitWidth )
	{
		originalNumber = originalNumber << (16 - bitWidth);
		originalNumber = originalNumber >> (16 - bitWidth);
		return originalNumber;
